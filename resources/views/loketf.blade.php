...
<div class="card-deck mb-3 mt-4">
    ...
    <div class="card mb-4 box-shadow">
        <div class="card-header text-center">
            <h2 class="my-0 font-weight-bold">Petugas Loket F</h2>
        </div>
        <table class="table">
            <thead>
                <tr>
                ...
                @foreach ($loketf as $data)
                    ...
                    >
                        ...
                        <td class="text-center d-flex justify-content-center">
                            ...
                            <audio id="loketf" src="/sound/loket/loketf.mp3"></audio>
                            <button onclick="playSequentialSounds(['{{$data->no_reg}}','{{$data->kd_dokter}}','loketf'])"  class="btn btn-primary" role="button" aria-disabled="true">Panggil</a>
                            <form action="{{route('inputadaloketf')}}" method="post">
                            @csrf
                                <input type="text" hidden name="kd_dokter" id="" value="{{ $data->kd_dokter}}">
                                <input type="text" hidden name="kd_poli" id="" value="{{ $data->kd_poli}}">
                                <input type="text" hidden name="no_rawat" id="" value="{{ $data->no_rawat}}">
                                <button type="submit" class="btn btn-success mx-2" role="button" aria-disabled="true">Ada</a>
                            </form>
                            <form action="{{route('inputtidakadaloketf')}}" method="post">
                                @csrf
                                <input type="text" hidden name="kd_dokter" id="" value="{{ $data->kd_dokter}}">
                                <input type="text" hidden name="kd_poli" id="" value="{{ $data->kd_poli}}">
                                <input type="text" hidden name="no_rawat" id="" value="{{ $data->no_rawat}}">
                                    <button type="submit" class="btn btn-danger" role="button" aria-disabled="true">Tidak Ada</a>
                            </form>
                        </td>
                    </tr>
                @endforeach
