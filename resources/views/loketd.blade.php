    ...
    <div class="card-deck mb-3 mt-4">
        ...
        <div class="card mb-4 box-shadow">
            <div class="card-header text-center">
                <h2 class="my-0 font-weight-bold">Petugas Loket D</h2>
            </div>
            <table class="table">
                <thead>
                    <tr>
                        ...
                        @elseif($data->status === '1')
                          style="background-color:#F97B22;" @endif>
                            ...
                            <td class="text-center d-flex justify-content-center">
                                ...
                                <audio id="loketd" src="/sound/loket/loketd.mp3"></audio>
                                <button onclick="playSequentialSounds(['{{ $data->no_reg }}','{{ $data->kd_dokter }}','loketd'])"
                                    class="btn btn-primary" role="button" aria-disabled="true">Panggil</a>
                                    <form action="{{ route('inputadaloketd') }}" method="post">
                                        @csrf
                                    ...
                                    <form action="{{ route('inputtidakadaloketd') }}" method="post">
                                        @csrf
                                        <input type="text" hidden name="kd_dokter" id=""
                                            value="{{ $data->kd_dokter }}">
                                        <input type="text" hidden name="kd_poli" id=""
